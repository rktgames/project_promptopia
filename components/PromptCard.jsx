    ...
    <div className="prompt_card">
      <div className="flex justify-between items-start gap-5">
        ...
        <div className="flex-1 flex justify-center items-center gap-3 cursor-pointer">
          <Link className="w-full" href={`/profile/${post.creator._id}?name=${post.creator.username}`}>
      ...
      {session?.user.id === post.creator._id && pathName === '/profile' && (
        <div className="mt-5 flex-center gap-4 border-t border-gray-100 pt-3">
          <p className="font-inter text-sm green_gradient cursor-pointer border rounded-md px-5 py-1.5" onClick={handleEdit}>Edit</p>
          <p className="font-inter text-sm orange_gradient cursor-pointer border rounded-md px-5 py-1.5" onClick={handleDelete}>Delete</p>
        </div>
      )}
